...
const TIPO_LABELS = {
  prazo:'Prazo', pagamento:'Pagamento', impostos:'Impostos',
  local:'Local', validade:'Validade', cond_gerais:'Cond. Gerais',
};
...
const Templates = {
  ...
  async load() {
    const { data } = await db.from('condition_templates').select('*').order('tipo').order('titulo');
    this._list = data || [];
    this._render();
  },
  ...
  _render() {
    ...
    el.innerHTML = this._list.map(t => `
      <div class="template-card">
        ...
        <div style="flex:1">
          <div class="tpl-titulo">${t.titulo}${t.is_default ? ' <span style="font-size:10px;color:var(--accent)">★ padrão</span>' : ''}</div>
          <div class="tpl-conteudo">${t.conteudo}</div>
        </div>
  ...
  async loadChips() {
    ...
    tipos.forEach(tipo => {
      ...
      el.innerHTML = items.map(t => `
        <span class="tpl-chip" onclick="Templates.applyChip('${tipo}','${t.conteudo.replace(/'/g,"\\'")}')">
          ${t.titulo}
        </span>`).join('');
    });
  },
  ...
  applyChip(tipo, conteudo) {
    const fieldMap = {
      prazo: 'f-prazo', pagamento: 'f-pagamento', impostos: 'f-impostos',
      local: 'f-local', validade: 'f-validade', cond_gerais: 'f-cond-gerais',
    };
    const el = document.getElementById(fieldMap[tipo]);
    if (el) { el.value = conteudo; UI.toast('Template aplicado', 'success'); }
  },
    ...
    UI.openModal(
      ...
      `<div class="form-group" style="margin-bottom:12px">
        ...
        <select id="tpl-new-tipo">
          ${Object.entries(TIPO_LABELS).map(([v,l]) => `<option value="${v}" ${t?.tipo===v?'selected':''}>${l}</option>`).join('')}
        </select>
      </div>
      <div class="form-group" style="margin-bottom:12px">
        <label>Título</label>
        <input type="text" id="tpl-new-titulo" value="${t?.titulo||''}" placeholder="Nome do template">
      </div>
      <div class="form-group" style="margin-bottom:12px">
        <label>Conteúdo</label>
        <textarea id="tpl-new-conteudo" rows="4">${t?.conteudo||''}</textarea>
      </div>
  ...
  async save(id = null) {
    ...
    if (!titulo || !conteudo) { UI.toast('Preencha título e conteúdo.', 'error'); return; }
  ...
  async delete(id) {
    UI.openModal('Remover Template', '<p>Tem certeza que deseja remover este template?</p>', async () => {
      await db.from('condition_templates').delete().eq('id', id);
      UI.closeModal();
